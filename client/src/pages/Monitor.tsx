...
import { Camera, Wifi, Plane, Eye, Navigation, AlertTriangle, Power, Home } from 'lucide-react';
...
// ============================================================
// Configuration - FIXED for Vite
// ============================================================

// Use Vite's import.meta.env instead of process.env
const BRIDGE_URL = import.meta.env.VITE_DRONE_BRIDGE_URL || 'http://localhost:5000';
const DETECTION_INTERVAL_MS = 300;
const DRONE_FOLLOW_INTERVAL_MS = 2000;
const DRONE_TRIGGER_COOLDOWN_MS = 60000;

// ============================================================
// Drone Bridge Service - Unified API
// ============================================================

class DroneBridgeService {
  private static instance: DroneBridgeService;
  private connectionStatus: 'connected' | 'connecting' | 'disconnected' = 'disconnected';
  private telemetry: any = null;
  private listeners: Set<(status: string, data: any) => void> = new Set();
  private bridgeUrl: string;

  private constructor() {
    this.bridgeUrl = import.meta.env.VITE_DRONE_BRIDGE_URL || 'http://localhost:5000';
  }

  static getInstance(): DroneBridgeService {
    if (!DroneBridgeService.instance) {
      DroneBridgeService.instance = new DroneBridgeService();
    }
    return DroneBridgeService.instance;
  }

  async connect(): Promise<boolean> {
    try {
      this.connectionStatus = 'connecting';
      this.notifyListeners('connecting', null);
      ...
      this.notifyListeners('disconnected', { error: String(error) });
      return false;
    }
  }
  ...
  async getHealth(): Promise<any> {
    try {
      ...
      console.error('[Drone] Health check failed:', error);
      return null;
    }
  }
  ...
  async getTelemetry(): Promise<any> {
    try {
      const response = await fetch(`${this.bridgeUrl}/telemetry`);
      ...
      console.error('[Drone] Telemetry failed:', error);
      return null;
    }
  }
  ...
  async sendCommand(command: string, params: any = {}): Promise<any> {
    try {
      ...
      this.notifyListeners('command', { command, result });
      return result;
    } catch (error) {
      console.error(`[Drone] Command '${command}' failed:`, error);
      throw error;
    }
  }

  // Convenience commands
  async takeoff(altitude: number = 15): Promise<any> {
    return this.sendCommand('takeoff', { altitude });
  }

  async land(): Promise<any> {
    return this.sendCommand('land');
  }

  async returnHome(): Promise<any> {
    return this.sendCommand('return_home');
  }

  async hover(): Promise<any> {
    return this.sendCommand('hover');
  }

  async followTarget(target_x: number, target_y: number, options: any = {}): Promise<any> {
    return this.sendCommand('follow_target', {
      target_x,
      target_y,
      velocity_dx: options.velocity_dx || 0,
      velocity_dy: options.velocity_dy || 0,
      track_id: options.track_id || 'unknown',
      confidence: options.confidence || 0,
      reason: options.reason || 'Following target',
      ...options
    });
  }

  async investigate(reason: string, target: any, confidence: number = 0): Promise<any> {
    return this.sendCommand('investigate', {
      reason,
      target,
      confidence,
      detection_type: target?.type || 'anonymous'
    });
  }
  ...
  getVideoFeedUrl(): string {
    return `${this.bridgeUrl}/video_feed`;
  }

  getSnapshotUrl(): string {
    return `${this.bridgeUrl}/snapshot`;
  }
...
const droneService = DroneBridgeService.getInstance();
...
export default function Monitor() {
  ...
  const [droneStatus, setDroneStatus] = useState<'idle' | 'following' | 'investigating' | 'error' | 'returning'>('idle');
  ...
  const [droneTelemetry, setDroneTelemetry] = useState<DroneStatus | null>(null);
  ...
  useEffect(() => {
    aiDetection.initialize().then(() => {
      ...
      console.log('[Monitor] AI Detection Engine v8.0 ready');
    });
    checkBridgeHealth();
  }, []);
  ...
  const checkBridgeHealth = async () => {
    try {
      const health = await droneService.getHealth();
      setBridgeConnected(health?.healthy || false);
      if (health?.healthy) {
        console.log('[Monitor] Drone bridge connected:', health);
      ...
      console.warn('[Monitor] Drone bridge not reachable');
    }
  };
  ...
  const startDroneCamera = () => {
    ...
    const poll = async () => {
      try {
        const snapshotUrl = droneService.getSnapshotUrl();
  ...
  // === DRONE COMMANDS ===
  const dispatchDrone = useCallback(async (target: TrackedObject, reason: string) => {
    const now = Date.now();
    ...
    if (!follow.isFollowing || follow.targetTrackId !== target.id) {
      ...
      setDroneMessage(`Following: ${reason} (${(target.confidence * 100).toFixed(0)}% confidence)`);

      console.log('[Monitor] 🚁 DISPATCHING DRONE:', { target: target.id, reason });

      try {
        await droneService.takeoff(15);
        console.log('[Monitor] ✅ Drone takeoff commanded');
        ...
        await droneService.followTarget(centerX, centerY, {
          track_id: target.id,
        ...
        console.log('[Monitor] ✅ Drone following target');

        setTimeout(async () => {
          try {
            await droneService.takePhoto();
            console.log('[Monitor] 📸 Verification photo taken');
          } catch (e) { /* ignore */ }
        }, 2000);

      } catch (err) {
        console.error('[Monitor] ❌ Drone dispatch failed:', err);
        setDroneStatus('error');
        setDroneMessage(`Dispatch error: ${err instanceof Error ? err.message : 'Unknown'}`);
      }
      return;
    }
    ...
    try {
      await droneService.followTarget(centerX, centerY, {
        track_id: target.id,
        confidence: target.confidence,
        reason: droneFollowRef.current.reason,
        velocity_dx: target.velocity.dx,
        velocity_dy: target.velocity.dy,
      });
    } catch (err) {
      console.warn('[Monitor] Follow update failed:', err);
    }
  }, []);
  ...
  const stopDroneFollow = useCallback(async () => {
    ...
    console.log('[Monitor] 🛑 Stopping drone follow');
    droneFollowRef.current = { 
      isFollowing: false, 
      targetTrackId: null, 
      lastGpsSent: 0, 
    ...
    setDroneMessage('Returning to home...');

    try {
      await droneService.hover();
      await new Promise(resolve => setTimeout(resolve, 2000));
      await droneService.returnHome();
      setDroneStatus('idle');
      setDroneMessage('');
      console.log('[Monitor] ✅ Drone returned home');
    } catch (err) {
      console.warn('[Monitor] Stop/return failed:', err);
      setDroneStatus('error');
      setDroneMessage('Return failed - manual intervention required');
    }
  }, []);
  ...
  const drawFrame = useCallback((tracked: TrackedObject[]) => {
    ...
    for (const obj of tracked) {
      ...
      if (droneFollowRef.current.isFollowing && droneFollowRef.current.targetTrackId === obj.id) {
        ...
        ctx.fillRect(bx, by + bh + 6, ftw + 12, 18);
        ctx.fillStyle = '#FFFFFF';
    ...
    ctx.fillText(`● ${useDroneCamera ? 'DRONE' : 'CAM'} | ${new Date().toLocaleTimeString()} | Objects: ${tracked.length} | FPS: ${fps}`, 10, ch - 16);
    ctx.textAlign = 'right';
    
    if (droneStatus === 'following') {
      ctx.fillStyle = '#FF4444';
      ctx.fillText('🚁 FOLLOWING', cw - 10, ch - 16);
    } else if (droneStatus === 'investigating') {
      ctx.fillStyle = '#FF8800';
      ctx.fillText('🔍 INVESTIGATING', cw - 10, ch - 16);
    } else if (droneStatus === 'error') {
      ctx.fillStyle = '#FF0000';
      ctx.fillText('⚠️ DRONE ERROR', cw - 10, ch - 16);
    } else {
      ctx.fillStyle = '#88FF88';
      ctx.fillText('⏸ STANDBY', cw - 10, ch - 16);
    }
    
    if (!bridgeConnected) {
      ctx.fillStyle = '#FF4400';
      ctx.textAlign = 'left';
      ctx.fillText('⚠️ Bridge disconnected', 10, ch - 16);
    }
    ctx.restore();
  }, [fps, droneStatus, useDroneCamera, bridgeConnected]);
  ...
  const runDetection = useCallback(async () => {
    ...
    try {
      ...
      if (bridgeConnected && result.shouldTriggerDrone && result.droneTarget) {
        await dispatchDrone(result.droneTarget, result.droneReason || 'Anonymous object detected');
      } else if (droneFollowRef.current.isFollowing) {
        const target = result.tracked.find(t => t.id === droneFollowRef.current.targetTrackId);
        if (target) {
          await dispatchDrone(target, droneFollowRef.current.reason);
        } else {
      ...
      console.error('[Monitor] Detection error:', error);
  ...
  }, [isMonitoring, useDroneCamera, drawFrame, sendToServer, dispatchDrone, stopDroneFollow, bridgeConnected]);
  ...
  const handleDroneTakeoff = async () => {
    try {
      await droneService.takeoff(15);
      setDroneMessage('Takeoff commanded');
      setTimeout(() => setDroneMessage(''), 3000);
    } catch (err) {
      setDroneMessage('Takeoff failed');
    }
  };

  const handleDroneLand = async () => {
    try {
      await droneService.land();
      setDroneMessage('Landing...');
      setTimeout(() => setDroneMessage(''), 3000);
    } catch (err) {
      setDroneMessage('Land failed');
    }
  };

  const handleDroneReturnHome = async () => {
    try {
      await droneService.returnHome();
      setDroneMessage('Returning home...');
      setTimeout(() => setDroneMessage(''), 3000);
    } catch (err) {
      setDroneMessage('Return home failed');
    }
  };
    ...
    <div className="p-4 space-y-4 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className={bridgeConnected ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}>
            <Wifi className="w-3 h-3 mr-1" /> {bridgeConnected ? 'Bridge Online' : 'Bridge Offline'}
          </Badge>
          {droneTelemetry && (
            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
              <Power className="w-3 h-3 mr-1" /> {droneTelemetry.battery}%
            </Badge>
          )}
        </div>
      </div>
      ...
      {droneStatus !== 'idle' && droneStatus !== 'error' && (
        <div className={`rounded-lg p-3 flex items-center gap-3 ${droneStatus === 'following' ? 'bg-red-100 border-2 border-red-400' : 'bg-yellow-100 border-2 border-yellow-400'}`}>
          <Navigation className={`w-6 h-6 ${droneStatus === 'following' ? 'text-red-600' : 'text-yellow-600'}`} />
          <div>
            <span className="font-bold">{droneStatus === 'following' ? '🚁 DRONE FOLLOWING: ' : '🔍 DRONE INVESTIGATING: '}</span>
            <span>{droneMessage}</span>
          </div>
          <Button size="sm" variant="outline" className="ml-auto" onClick={stopDroneFollow}>
            <Home className="w-4 h-4 mr-1" /> Recall
          </Button>
        </div>
      )}

      {droneStatus === 'error' && (
        <div className="bg-red-200 border-2 border-red-600 rounded-lg p-3 flex items-center gap-3">
          <AlertTriangle className="w-6 h-6 text-red-700" />
          <div>
            <span className="font-bold text-red-800">⚠️ DRONE ERROR: </span>
            <span className="text-red-700">{droneMessage}</span>
          </div>
          <Button size="sm" variant="destructive" className="ml-auto" onClick={() => setDroneStatus('idle')}>
            Dismiss
          </Button>
        </div>
      )}
            ...
            <CardContent className="pt-0 space-y-3">
              ...
              {useDroneCamera && isMonitoring && !droneVideoConnected && (
                <div className="text-sm text-yellow-700 bg-yellow-50 px-3 py-2 rounded border border-yellow-200">
                  Connecting to drone camera at {droneService.getSnapshotUrl()}... Make sure the bridge is running.
                </div>
              )}

              <div className="flex flex-wrap gap-2 justify-center">
                {!isMonitoring ? (
                  <Button onClick={startMonitoring} size="lg" className="bg-green-600 hover:bg-green-700 text-white">
                ...
                {bridgeConnected && (
                  <>
                    <Button size="sm" variant="outline" onClick={handleDroneTakeoff} disabled={isMonitoring}>
                      <Plane className="w-4 h-4 mr-1" /> Takeoff
                    </Button>
                    <Button size="sm" variant="outline" onClick={handleDroneLand} disabled={isMonitoring}>
                      Land
                    </Button>
                    <Button size="sm" variant="outline" onClick={handleDroneReturnHome} disabled={isMonitoring}>
                      <Home className="w-4 h-4 mr-1" /> RTH
                    </Button>
                  </>
                )}
              </div>
        ...
        <div className="space-y-4">
                    ...
                    <div key={obj.id} className="flex items-center justify-between p-2 rounded bg-secondary/50 border-l-4"
                      ...
                      <div className="text-right">
                        ...
                        {droneFollowRef.current.targetTrackId === obj.id && (
                          <Badge variant="destructive" className="text-xs animate-pulse">🚁 FOLLOWING</Badge>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader className="py-3">
              <CardTitle className="text-lg flex items-center gap-2"><Plane className="w-4 h-4" /> Drone Status</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-3 h-3 rounded-full ${
                  droneStatus === 'idle' ? 'bg-green-500' : 
                  droneStatus === 'error' ? 'bg-red-500' :
                  droneStatus === 'returning' ? 'bg-orange-500' : 'bg-red-500 animate-pulse'
                }`} />
                <span className="font-medium capitalize">{droneStatus}</span>
                {droneTelemetry && (
                  <span className="text-xs text-muted-foreground ml-auto">
                    {droneTelemetry.armed ? '⚡ Armed' : '🔒 Disarmed'}
                  </span>
                )}
              </div>
              ...
              {droneTelemetry && (
                <div className="mt-3 grid grid-cols-2 gap-1 text-xs">
                  <div className="flex justify-between"><span>Battery</span><span className="font-mono">{droneTelemetry.battery}%</span></div>
                  <div className="flex justify-between"><span>Signal</span><span className="font-mono">{droneTelemetry.signal}%</span></div>
                  <div className="flex justify-between"><span>Mode</span><span className="font-mono">{droneTelemetry.flightMode}</span></div>
                  <div className="flex justify-between"><span>GPS</span><span className="font-mono">{droneTelemetry.gps.satellites} sats</span></div>
                </div>
              )}
